...
class OAuthService {
    ...
    async getAuthUrlWithLocalServer(provider) {
    ...
    async handleOAuthFlow(provider) {
    ...
    async handleGmailCallback(code, config) {
    ...
    async handleOutlookCallback(code, config) {
    ...
    async refreshGmailToken(refreshToken, config) {
        const tokenResponse = await fetch('https://oauth2.googleapis.com/token', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
    ...
    async refreshOutlookToken(refreshToken, config) {
        const tokenResponse = await fetch('https://login.microsoftonline.com/common/oauth2/v2.0/token', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
