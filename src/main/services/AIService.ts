import { AIResponse, AIMessageRequest, EmailMessage } from '@/shared/types';
...
export class AIService {
  ...
  private async sendOllamaMessage(request: AIMessageRequest): Promise<AIResponse> {
    const systemPrompt = this.buildSystemPrompt(request.emails);
    const userPrompt = `${request.context}\n\nUser Question: ${request.message}`;
  ...
  private async sendHuggingFaceMessage(request: AIMessageRequest): Promise<AIResponse> {
  ...
  private async sendCohereMessage(request: AIMessageRequest): Promise<AIResponse> {
  ...
  private async sendGroqMessage(request: AIMessageRequest): Promise<AIResponse> {
  ...
  private async sendTogetherMessage(request: AIMessageRequest): Promise<AIResponse> {
...
Please provide helpful, contextual responses based on the email content and user's request.`;
  ...
  async summarizeEmail(email: EmailMessage, model: 'openai' | 'claude' | 'ollama' | 'huggingface' | 'cohere' | 'groq' | 'together' = 'openai'): Promise<string> {
  ...
  async draftReply(email: EmailMessage, model: 'openai' | 'claude' | 'ollama' | 'huggingface' | 'cohere' | 'groq' | 'together' = 'openai'): Promise<string> {
    const request: AIMessageRequest = {
  ...
  async extractActionItems(email: EmailMessage, model: 'openai' | 'claude' | 'ollama' | 'huggingface' | 'cohere' | 'groq' | 'together' = 'openai'): Promise<string> {
    const request: AIMessageRequest = {
  ...
  async analyzeTone(email: EmailMessage, model: 'openai' | 'claude' | 'ollama' | 'huggingface' | 'cohere' | 'groq' | 'together' = 'openai'): Promise<string> {
    const request: AIMessageRequest = {
