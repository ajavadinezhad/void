...
import { EmailAccount, EmailFolder, EmailMessage, ComposeEmail } from '@/shared/types';

export class EmailService {
  private gmailApiBase = 'https://gmail.googleapis.com/gmail/v1/users/me';

  constructor(
    private databaseService: DatabaseService,
    private oauthService?: any // OAuthService type
  ) {}

  // Gmail API Operations for syncing
  async syncFolder(accountId: number, folderId: number): Promise<void> {
  ...
  async refreshFolders(accountId: number): Promise<void> {
    const account = await this.getAccount(accountId);
  ...
  async syncAllFolders(accountId: number): Promise<void> {
  ...
  private async syncAllGmailFolders(account: EmailAccount): Promise<void> {
  ...
  private async syncGmailFolderByLabel(account: EmailAccount, folder: any): Promise<void> {
  ...
  private async syncGmailFolder(account: EmailAccount, folderId: number): Promise<void> {
  ...
  private async ensureGmailFolders(accountId: number): Promise<void> {
    const account = await this.getAccount(accountId);
  ...
  private async refreshAccountToken(accountId: number): Promise<string | null> {
    try {
      const account = await this.getAccount(accountId);
  ...
  private async fetchGmailLabels(oauthToken: string, accountId?: number): Promise<any[]> {
  ...
  private mapGmailLabelToFolderType(labelId: string): 'inbox' | 'sent' | 'drafts' | 'trash' | 'archive' | 'custom' {
    switch (labelId) {
      case 'INBOX':
        return 'inbox';
      case 'SENT':
  ...
  private getDisplayNameForLabel(label: any): string {
  ...
  private async createDefaultGmailFolders(accountId: number): Promise<void> {
    // Create default Gmail folders if they don't exist
    const defaultFolders = [
      { name: 'INBOX', path: 'INBOX', type: 'inbox' as const },
      { name: 'Sent', path: 'SENT', type: 'sent' as const },
      { name: 'Drafts', path: 'DRAFTS', type: 'drafts' as const },
      { name: 'Trash', path: 'TRASH', type: 'trash' as const }
    ];
  ...
  private async processGmailMessage(accountId: number, folderId: number, messageId: string): Promise<void> {
    try {
      const account = await this.getAccount(accountId);
      if (!account || !account.oauthToken) {
        throw new Error('Account not found or no OAuth token');
      ...
      const emailMessage: Omit<EmailMessage, 'id' | 'createdAt'> = {
  ...
  private async syncImapFolder(account: EmailAccount, folderId: number): Promise<void> {
            ...
            msg.once('end', async () => {
              try {
                ...
                const gmailThreadId = messageAttributes?.['x-gm-thrid']
                  ...
                  : undefined;
                const derivedFromHeaders = (parsed as any).inReplyTo
                  || (Array.isArray((parsed as any).references) && (parsed as any).references[0])
                  || parsed.messageId
                  || `thread-${Date.now()}`;
                const emailMessage: Omit<EmailMessage, 'id' | 'createdAt'> = {
  ...
  async sendMessageViaGmail(composeEmail: ComposeEmail & { accountId: number }): Promise<boolean> {
  ...
  private async getGmailMessagesByLabel(accountId: number, labelId: string, maxResults: number = 50): Promise<any[]> {
    const account = await this.getAccount(accountId);
    if (!account || !account.oauthToken) {
      throw new Error('Account not found or no OAuth token');
    }
  ...
  async getGmailMessages(accountId: number, maxResults: number = 100): Promise<any[]> {
    const account = await this.getAccount(accountId);
    if (!account || !account.oauthToken) {
  ...
  private createGmailMessage(composeEmail: ComposeEmail & { from: string }): string {
    const headers = [
  ...
  async sendMessage(composeEmail: ComposeEmail & { accountId: number }): Promise<boolean> {
    const account = await this.getAccount(composeEmail.accountId);
    if (!account) {
      throw new Error('Account not found');
    }
  ...
  private createSmtpTransporter(account: EmailAccount): nodemailer.Transporter {
    const config = {
  ...
  // Helper methods
  private async getAccount(id: number): Promise<EmailAccount | null> {
  ...
  private async getFolder(id: number): Promise<EmailFolder | null> {
